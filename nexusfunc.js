...
async function fetchCourses() {
  ...
  const response = await fetch('https://elbert-api-qa.azurewebsites.net/api/readonly/courses/NEXus - The Nursing Education Xchange');
...
function displayCoursesWithoutPagination(courses) {
  ...
  courses.forEach(course => {
    ...
    courseLink.href = `/coursedetailsnexus?courseCode=${course.code}`;
...
function displayCourses(courses, page = 1, rows = 10) {
    ...
    paginatedItems.forEach(course => {
        ...
        const courseLink = document.createElement('a');
          courseLink.href = `/coursedetailsnexus?courseCode=${course.code}&providerName=${course.providerName}`;
        courseLink.className = 'course-card-link';
